...
const CurrentTransactionOnline = ({ startLoading, stopLoading }) => {
      ...
      if (user?.length > 0) {
        try {
          ...
          const response = await FetchData(
            `payment/admin/get-online-payments/${transactionId}`,
            "get"
          );
    ...
    <div className="flex justify-evenly items-center flex-col">
      <div>
        ...
        <h1>
          ...
          <span className="text-xl font-bold">{currentTransaction?._id}</span>
        </h1>
      ...
      <div className="flex justify-evenly items-center px-4 py-10 bg-white shadow-m rounded-xl w-full ">
        <div className="w-full flex justify-center flex-col items-center">
          ...
          {[
            ...
            { label: "Created At", value: currentTransaction?.createdAt },
            {
              label: "Razorpay Order Id",
              value: currentTransaction?.razorpay_order_id,
            },
            {
              label: "Razorpay Payment Id",
              value: currentTransaction?.razorpay_payment_id,
            },
            {
              label: "Order Id",
              value: order?._id || "No data found",
            },
            // { label: "Status", value: currentTransaction?.status },
          ].map((item, index) => (
            <h2
              key={index}
...
export default LoadingUI(CurrentTransactionOnline);
